...
public abstract class Document {
	...
	protected static int countSyllables(String word)
	{
		...
		if(Character.toLowerCase(word.charAt(word.length()-1)) == 'e') {
			max--;
		}
		for(int i=0;i<max;i++) {
	...
	public double getFleschScore()
	{
		...
		double fleshScore = 0;
		fleshScore = 206.835 - 1.015*(((double)getNumWords())/(double)(getNumSentences()))-84.6*(((double)getNumSyllables())/((double)getNumWords()));
		//fleshScore = getNumSyllables();
	    return fleshScore;
	}
